...
def fednova_aggregate(global_weights, client_weights, client_sizes, client_steps):
    ...
    for layer_idx, gw in enumerate(global_weights):
        ...
        for k in range(len(client_weights)):
            ...
            d_k = (gw - wk) / tau_k
            d_sum += p[k] * d_k
        new_weights.append(gw - tau_eff * d_sum)
    return new_weights
...
drone_nova_model.save("outputs/drone_lstm_fednova.keras")
kdd_nova_model.save("outputs/kdd_lstm_fednova.keras")
...
res_df.to_csv("outputs/fednova_results.csv", index=False)
print("\nSaved outputs/fednova_results.csv")
print(res_df.to_string(index=False))

fig, axes = plt.subplots(3, 2, figsize=(14, 14))
fig.suptitle("Federated LSTM (FedNova) Intrusion Detection Results", fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.savefig("outputs/fednova_results.png", dpi=150, bbox_inches='tight')
print("Saved outputs/fednova_results.png")
plt.close()
